...
static inline int get_inject_rma_size_opt(nccl_net_ofi_device_t *base_dev,
					  size_t *max_write_inline_size)
{
	...
	ret = fi_getopt(&ep->rails[0].ofi_ep->fid, FI_OPT_ENDPOINT, FI_OPT_INJECT_RMA_SIZE,
				 ...
				 nccl_ofi_properties_t *props)
{
	...
	ret = get_inject_rma_size_opt(base_dev, &max_write_inline_size);
	if (ret == 0) {
		props->max_write_inline_size = max_write_inline_size;
	} else if (ret == -FI_ENOPROTOOPT) {
		ret = 0;
	} else {
		NCCL_OFI_WARN("Failed to create RDMA device property");
	}
 ...
 */
static inline rdma_req_rma_op_data_t *get_rma_op_data(nccl_net_ofi_rdma_req_t *req,
...
static const char *req_type_str(nccl_net_ofi_rdma_req_type_t type)
{
	...
	case NCCL_OFI_RDMA_WRITE:
		return "WRITE";
	case NCCL_OFI_RDMA_WRITE_INLINE:
		return "WRITE_INLINE";
	...
	for (comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		if (comp_flags & FI_SEND) {
			...
			case NCCL_OFI_RDMA_WRITE: {
				...
				rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_WRITE);
				ret = inc_req_completion(req, 0, rma_op_data->total_num_compls);
				break;
			}
			case NCCL_OFI_RDMA_WRITE_INLINE: {
				/* Local-initiated RMA write inline is complete */

				rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_WRITE_INLINE);
				ret = inc_req_completion(req, 0, rma_op_data->total_num_compls);
				break;
			}
			default:
			...
			case NCCL_OFI_RDMA_READ: {
				...
				rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_READ);
				ret = inc_req_completion(req, 0, rma_op_data->total_num_compls);
				break;
			}
			default:
...
{
	rdma_req_rma_op_data_t *rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_READ);
	struct fid_mr *rail_mr_handle = rma_op_data->buff_mr_handle->mr[0];
...
/*
 * @brief	Free write_inline request
 */
static inline int free_write_inline_req(nccl_net_ofi_rdma_req_t *req,
				      bool dec_inflight_reqs)
{
	assert(req->type == NCCL_OFI_RDMA_WRITE_INLINE);
	nccl_net_ofi_rdma_send_comm_t *s_comm =
		(nccl_net_ofi_rdma_send_comm_t *)req->comm;
...
static int test(nccl_net_ofi_req_t *base_req, int *done, int *size)
{
	...
	*done = 0;
	assert(req->type == NCCL_OFI_RDMA_WRITE ||
	       req->type == NCCL_OFI_RDMA_WRITE_INLINE ||
			    ...
			    nccl_net_ofi_rdma_req_type_t req_type)
{
	...
	rdma_req_rma_op_data_t *rma_op_data = get_rma_op_data(req, req_type);
	...
	rma_op_data->buff_mr_handle = buff_mr_handle;
			       ...
			       nccl_net_ofi_rdma_req_t **ret_req)
{
	*ret_req = NULL;
	...
	init_rma_op_req(req, &r_comm->base.base, buff, size, buff_mr_handle, remote_buff,
			remote_mr_key, NCCL_OFI_RDMA_READ);
				...
				nccl_net_ofi_rdma_req_t **ret_req)
{
	*ret_req = NULL;
	...
	init_rma_op_req(req, &s_comm->base.base, buff, size, buff_mr_handle, remote_buff,
			remote_mr_key, NCCL_OFI_RDMA_WRITE);
...
static int alloc_rdma_write_inline_req(nccl_net_ofi_rdma_send_comm_t *s_comm,
				     nccl_net_ofi_rdma_ep_t *ep,
				     void *buff, size_t size,
				     uint64_t remote_buff,
				     uint64_t remote_mr_key,
				     nccl_net_ofi_rdma_req_t **ret_req)
{
	*ret_req = NULL;
	...
	nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle = NULL;
	req->free = free_write_inline_req;
	init_rma_op_req(req, &s_comm->base.base, buff, size, buff_mr_handle, remote_buff,
			remote_mr_key, NCCL_OFI_RDMA_WRITE_INLINE);
...
static int post_rma_write(nccl_net_ofi_rdma_req_t *req)
{
	...
	rdma_req_rma_op_data_t *rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_WRITE);
	struct fid_mr *rail_mr_handle = rma_op_data->buff_mr_handle->mr[0];
	void *desc = fi_mr_desc(rail_mr_handle);

	ssize_t rc;
	/* Post RMA write */
	rc = fi_write(comm_rail->local_ep, rma_op_data->buff,
		      rma_op_data->buff_len, desc,
		      comm_rail->remote_addr,
		      rma_op_data->remote_buff,
		      rma_op_data->remote_mr_key, req);
...
static int post_rma_write_inline(nccl_net_ofi_rdma_req_t *req)
{
	nccl_net_ofi_rdma_send_comm_t *s_comm = (nccl_net_ofi_rdma_send_comm_t *)req->comm;
	nccl_net_ofi_rdma_send_comm_rail_t *comm_rail = get_send_comm_rail(s_comm, 0);
	rdma_req_rma_op_data_t *rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_WRITE_INLINE);
	ssize_t rc;
...
static int send_progress(nccl_net_ofi_rdma_req_t *req)
{
	...
	} else if (req->type == NCCL_OFI_RDMA_WRITE_INLINE) { // Post RMA write using fi_writemsg
		ret = post_rma_write_inline(req);
		if (ret == 0) {
			rdma_req_rma_op_data_t *rma_op_data = get_rma_op_data(req, NCCL_OFI_RDMA_WRITE_INLINE);
			// Successfully sent the xfer with this rail
			rma_op_data->xferred_rail_id++;
			 ...
			 nccl_net_ofi_mr_handle_t *mhandle, nccl_net_ofi_req_t **base_req)
{
	...
	/* Try posting RMA write for received RDMA control messages */
	if (have_ctrl || eager) {
...
{
	nccl_net_ofi_rdma_mr_handle_t *mr_handle = (nccl_net_ofi_rdma_mr_handle_t *)mhandle;
	*mr_key = fi_mr_key(mr_handle->mr[0]);
	return 0;
}

/**
 * @brief	write inline using RMA. This "interface function" is called, indirectly, from
 *       	the application
 */
		     ...
		     uint64_t dest, uint64_t mr_key, nccl_net_ofi_req_t ** base_req)
{
	...
	ret = alloc_rdma_write_req(s_comm, ep, src,
				   size, mr_handle, dest, mr_key, &req);
	if (OFI_UNLIKELY(ret != 0)) {
		goto error;
	}
...
/**
 * @brief	Write inline using DMA writemsg interface. This "interface function" is called, indirectly, from
 *       	the application
 */
			  ...
			  uint64_t dest, uint64_t mr_key, nccl_net_ofi_req_t ** base_req)
{
	...
	ret = alloc_rdma_write_inline_req(s_comm, ep, src,
					size, dest, mr_key, &req);
	if (OFI_UNLIKELY(ret != 0)) {
		goto error;
	}
	...
	NCCL_OFI_TRACE_WRITE_INLINE(req, base_req);
