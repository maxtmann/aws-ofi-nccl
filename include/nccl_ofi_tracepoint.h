...
#define NCCL_OFI_TRACE_WRITE_INLINE(request, nccl_req) do { \
	lttng_ust_tracepoint(nccl_ofi_plugin, WriteInline, request, nccl_req); \
	NCCL_OFI_TRACE_WRITE_INLINE_NVTX(request, nccl_req); \
} while(0)
